...
rlplot = zeros(1,n_trays+1);
rvplot = zeros(1,n_trays+1);
xlplot = zeros(1,n_trays+1);
xvplot = zeros(1,n_trays+1);

% Get the values at the flash (feed tray)
Tplot([end-3:end])  = [T4 T3 T2 T1];
splot([end-3:end])  = [s4 s3 s2 s1];
xplot([end-3:end])  = [c4(N2) c(N2) c(N2) c(N2)];
...
for n = 0.7:0.001:0.999
    ...
    rlplot(1)       = rx_out;
    rvplot(1)       = ry_out;
    xlplot(1)       = x_out_mole(N2);
    xvplot(1)       = y_out_mole(N2);
    ...
    for i = 2:n_trays+1
        ...
        rlplot(i)       = r_liq_below;
        rvplot(i)       = r_vap_above;
        xlplot(i)       = x_liq_below(N2);
        xvplot(i)       = y_vap_above(N2);
    ...
    if abs(y_bottray(N2) - y4(N2)) < 0.01
        disp('Success!')
        break
    end
end
