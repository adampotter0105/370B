...
while it < max_it
        ...
        i_err = Phi_term(2) - V;
        delta_i = min( abs(i_err/dedi), i_max/10 ) * sign(i_err/dedi); % max out steps
        i_guess = i_guess - delta_i;
        
        % Bisection if out of bounds
        if i_guess > 1.5*i_max
            i_guess = 0.5*(1.5*i_max - i_prev) + i_prev; % PROBLEM HERE
        elseif i_guess <= 0
            i_guess = i_prev/2;
        end
...
fprintf("button_FC Error: Failed to find current to requested voltage! \n")
% 
% 
% % Dynamic Parameters
% T_range = 700+273.15:100:1200+273.15;
