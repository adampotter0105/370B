...
% Get fuel-specific exergy and flow exergy at each station 
gas = GRI30();
set(gas,'T',Ta1,'P',Pa1,'X',xair);
EXa1 = exergy_mass(gas)*mdot_air/mdot_fuel/1e6;
FXa1 = flowExergy_mass(gas)*mdot_air/mdot_fuel/1e6;
gas = GRI30();
set(gas,'T',Ta2,'P',Pa2,'X',xair);
EXa2 = exergy_mass(gas)*mdot_air/mdot_fuel/1e6;
FXa2 = flowExergy_mass(gas)*mdot_air/mdot_fuel/1e6;
gas = GRI30();
set(gas,'T',Tf1,'P',Pf1,'X',xfuel);
EXf1 = exergy_mass(gas)/1e6;
FXf1 = flowExergy_mass(gas)/1e6;
gas = GRI30();
set(gas,'T',Tf2,'P',Pf2,'X',xfuel);
EXf2 = exergy_mass(gas)/1e6;
FXf2 = flowExergy_mass(gas)/1e6;
gas = GRI30();
set(gas,'T',Tm3,'P',Pm3,'X',xmix);
EXm3 = exergy_mass(gas)*mdot_mix/mdot_fuel/1e6;
FXm3 = flowExergy_mass(gas)*mdot_mix/mdot_fuel/1e6;
gas = GRI30();
set(gas,'T',Tm4,'P',Pm4,'X',xmix);
EXm4 = exergy_mass(gas)*mdot_mix/mdot_fuel/1e6;
FXm4 = flowExergy_mass(gas)*mdot_mix/mdot_fuel/1e6;
gas = GRI30();
set(gas,'T',Tm5,'P',Pm5,'X',xmix);
EXm5 = exergy_mass(gas)*mdot_mix/mdot_fuel/1e6;
FXm5 = flowExergy_mass(gas)*mdot_mix/mdot_fuel/1e6;
gas = GRI30();
set(gas,'T',Tm6,'P',Pm6,'X',xmix);
EXm6 = exergy_mass(gas)*mdot_mix/mdot_fuel/1e6;
FXm6 = flowExergy_mass(gas)*mdot_mix/mdot_fuel/1e6;
gas = GRI30();
set(gas,'T',Tm7,'P',Pm7,'X',xmix);
EXm7 = exergy_mass(gas)*mdot_mix/mdot_fuel/1e6;
FXm7 = flowExergy_mass(gas)*mdot_mix/mdot_fuel/1e6;
gas = GRI30();
set(gas,'T',Tm8,'P',Pm8,'X',xmix);
EXm8 = exergy_mass(gas)*mdot_mix/mdot_fuel/1e6;
FXm8 = flowExergy_mass(gas)*mdot_mix/mdot_fuel/1e6;

w = Water();
set(w,'T',Tw1,'P',Pw1);
EXw1 = exergy_mass(w)*mdot_water/mdot_fuel/1e6;
FXw1 = flowExergy_mass(w)*mdot_water/mdot_fuel/1e6;
w = Water();
set(w,'T',Tw2,'P',Pw2);
EXw2 = exergy_mass(w)*mdot_water/mdot_fuel/1e6;
FXw2 = flowExergy_mass(w)*mdot_water/mdot_fuel/1e6;
w = Water();
set(w,'P',Pw3,'Vapor',0);
EXw3 = exergy_mass(w)*mdot_water/mdot_fuel/1e6;
FXw3 = flowExergy_mass(w)*mdot_water/mdot_fuel/1e6;
w = Water();
set(w,'P',Pw4,'Vapor',1);
EXw4 = exergy_mass(w)*mdot_water/mdot_fuel/1e6;
FXw4 = flowExergy_mass(w)*mdot_water/mdot_fuel/1e6;
w = Water();
set(w,'T',Tw5,'P',Pw5);
EXw5 = exergy_mass(w)*mdot_water/mdot_fuel/1e6;
FXw5 = flowExergy_mass(w)*mdot_water/mdot_fuel/1e6;
w = Water();
set(w,'P',Pw6, 'Vapor',0.88);
EXw6 = exergy_mass(w)*mdot_water/mdot_fuel/1e6;
FXw6 = flowExergy_mass(w)*mdot_water/mdot_fuel/1e6;
w = Water();
set(w,'P',Pw7,'Vapor',0);
EXw7 = exergy_mass(w)*mdot_water/mdot_fuel/1e6;
FXw7 = flowExergy_mass(w)*mdot_water/mdot_fuel/1e6;
w = Water();
set(w,'T',Tw8,'P',Pw8);
EXw8 = exergy_mass(w)*mdot_water/mdot_fuel/1e6;
FXw8 = flowExergy_mass(w)*mdot_water/mdot_fuel/1e6;

% Plot fuel-specific exergy vs. station number
Station = [1:8];
figure(2)
hold on
plot(Station(1:3),[EXa1 EXa2 EXm3],'bo-','DisplayName','Air')
plot(Station(1:3),[EXf1 EXf2 EXm3],'ro-','DisplayName','Fuel')
plot(Station(3:8),[EXm3 EXm4 EXm5 EXm6 EXm7 EXm8],'ko-','DisplayName','Gas Mix')
plot(Station(1:8),[EXw1 EXw2 EXw3 EXw4 EXw5 EXw6 EXw7 EXw8],'go-','DisplayName','Steam')
legend()
xlabel('Station');
ylabel('Fuel-Specific Exergy (MJ/kg_{fuel}-K)');
hold off

% Plot fuel-specific flow exergy vs. station number
Station = [1:8];
figure(3)
hold on
plot(Station(1:3),[FXa1 FXa2 FXm3],'bo-','DisplayName','Air')
plot(Station(1:3),[FXf1 FXf2 FXm3],'ro-','DisplayName','Fuel')
plot(Station(3:8),[FXm3 FXm4 FXm5 FXm6 FXm7 FXm8],'ko-','DisplayName','Gas Mix')
plot(Station(1:8),[FXw1 FXw2 FXw3 FXw4 FXw5 FXw6 FXw7 FXw8],'go-','DisplayName','Steam')
legend()
xlabel('Station');
ylabel('Fuel-Specific Exergy Flow (MJ/kg_{fuel}-K)');
hold off

% Extensive exergy analysis
X_des_fuel_comp     = (FXf1*mdot_fuel - FXf2*mdot_fuel + W_fuel_comp/1e6);
X_des_air_comp      = (FXa1*mdot_fuel - FXa2*mdot_fuel + W_air_comp/1e6);
X_des_premixer      = (FXf2*mdot_fuel + FXa2*mdot_fuel - FXm3*mdot_fuel);
X_des_combustor     = (FXm3*mdot_fuel - FXm4*mdot_fuel);
X_des_gas_turbine   = (FXm4*mdot_fuel - FXm5*mdot_fuel - W_gas_turb/1e6);
X_des_HRSG          = (FXm5*mdot_fuel + FXw2*mdot_fuel - FXm8*mdot_fuel - FXw5*mdot_fuel);
X_des_feed_pump     = (FXw1*mdot_fuel - FXw2*mdot_fuel + W_feed_pump/1e6);
X_des_steam_turbine = (FXw5*mdot_fuel - FXw6*mdot_fuel - W_steam_turb/1e6);
X_des_condenser     = (FXw6*mdot_fuel - FXw7*mdot_fuel);
X_des_condenser_pump = (FXw7*mdot_fuel - FXw8*mdot_fuel + W_cond_pump/1e6);
X_des_cold_storage  = (FXw8*mdot_fuel - FXw1*mdot_fuel);
X_des_flue_gas      = (FXm8*mdot_fuel);
X_des_total         = X_des_fuel_comp + X_des_air_comp + X_des_premixer + X_des_combustor + X_des_gas_turbine + X_des_HRSG + X_des_feed_pump + X_des_steam_turbine + X_des_condenser + X_des_condenser_pump + X_des_cold_storage + X_des_flue_gas;
X_des_list          = [X_des_fuel_comp X_des_air_comp X_des_premixer X_des_combustor X_des_gas_turbine X_des_HRSG X_des_feed_pump X_des_steam_turbine X_des_condenser X_des_condenser_pump X_des_cold_storage X_des_flue_gas]./X_des_total*100;
Eff_gas_turbine     = W_gas_net/(mdot_fuel*LHV_fuel);
Eff_exergy          = W_gas_net/1e6/(FXf1*mdot_fuel + FXa1*mdot_fuel);

% check W_gas_net
% Xm5_flow Xm6_flow]/result.mdot_fuel/1e6

% Plot exergy destruction bar graph
figure(4)
hold on
X_bar = categorical({'Fuel Compressor','Air Compressor','Premixer','Combustor','Gas Turbine','HRSG','Feed Pump','Steam Turbine','Condenser','Condenser Pump','Cold Storage','Flue Gas'});
X_bar = reordercats(X_bar,{'Fuel Compressor','Air Compressor','Premixer','Combustor','Gas Turbine','HRSG','Feed Pump','Steam Turbine','Condenser','Condenser Pump','Cold Storage','Flue Gas'});
bar(X_bar, X_des_list)
xlabel('Conversion Device')
ylabel('Exergy Loss (%)')
text(1,30,sprintf('LHV Efficiency: %.1f%%', Eff_gas_turbine*100))
text(1,20,sprintf('Exergy Efficiency: %.1f%%', Eff_exergy*100))
hold off
